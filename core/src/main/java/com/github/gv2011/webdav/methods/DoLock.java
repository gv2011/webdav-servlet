...
public class DoLock extends AbstractMethod {
    ...
    public DoLock(final IWebdavStore store, final IResourceLocks resourceLocks,
    ...
    public void execute(final ITransaction transaction, final HttpServletRequest req,
        ...
        if (_readOnly) {
            ...
            if (_userAgent != null && _userAgent.indexOf("Darwin") != -1) {
                ...
                final String timeString = new Long(System.currentTimeMillis())
                        .toString();
                _lockOwner = _userAgent.concat(timeString);
            }

            final String tempLockOwner = "doLock" + System.currentTimeMillis()
                    + req.toString();
    ...
    private void doLock(final ITransaction transaction, final HttpServletRequest req,
            final HttpServletResponse resp) throws IOException, LockFailedException {
    ...
    private void doLocking(final ITransaction transaction, final HttpServletRequest req,
    ...
    private void doNullResourceLock(final ITransaction transaction,
    ...
    private void doRefreshLock(final ITransaction transaction,
            final HttpServletRequest req, final HttpServletResponse resp)
     ...
     * Executes the LOCK
     */
    private void executeLock(final ITransaction transaction, final HttpServletRequest req,
            final HttpServletResponse resp) throws LockFailedException, IOException,
            ServletException {
        ...
        if (_macLockRequest) {
            ...
            if (getLockInformation(transaction, req, resp)) {
                ...
                if (lockSuccess) {
                    ...
                    final LockedObject lo = _resourceLocks.getLockedObjectByPath(
                            transaction, _path);
                    if (lo != null) {
     ...
     */
    private boolean getLockInformation(final ITransaction transaction,
            final HttpServletRequest req, final HttpServletResponse resp)
        ...
        try {
            final Document document = documentBuilder.parse(new InputSource(req
                    .getInputStream()));
     ...
     */
    private void generateXMLReport(final ITransaction transaction,
            final HttpServletResponse resp, final LockedObject lo) throws IOException {
     ...
     */
    private void doMacLockRequestWorkaround(final ITransaction transaction,
            final HttpServletRequest req, final HttpServletResponse resp)
     ...
     */
    private void sendLockFailError(final ITransaction transaction,
            final HttpServletRequest req, final HttpServletResponse resp)
            throws IOException {
